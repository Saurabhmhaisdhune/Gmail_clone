...
export default function Tabss() {
  ...
  const handleDelete = (name)=>{
    axios.delete('https://gmailclon.herokuapp.com/gmail_data/'+name)
    .then((data)=>getData());
  }
  useEffect(()=>{
    getData();
  ...
  <div className='above-main-tab'>
    <div className='checkbox-arrow'>
      <input type='checkbox' className='check-box'/><IoMdArrowDropdown  className='check-arrow'/>
     ...
     typography: 'body1'
      ...
      <TabContext value={value}>
        ...
        <TabPanel value="1">
          <div >
            {mails.map((value)=>{
              return <div className='mail-recived' key={value.name} >
                <div>
           <label className='tabs-icon-checkbox'><input type='checkbox' className='tabs-check-box'/></label>
           <label className='tabs-icons'><AiOutlineStar/></label>
           </div>
           <div onClick={()=>{navigate('/mailbox/'+value.name)}}>
           ...
           <label className='tabs-icon'><BiArchiveIn/></label>
           <label className='tabs-icon' onClick={()=>handleDelete(value.name)}><RiDeleteBin6Line/></label>
           <label className='tabs-icon'><TbMailOpened/></label>
           <label className='tabs-icon'><AiOutlineClockCircle/></label>
           </div>
           </div>
           })}
          </div>
